class MovesController < ApplicationController
  ...
  # 技の削除
  def skill_destroy
    @moves = Move.where(user_id: params[:user_id], title: params[:title])
    @move = Move.find_by(user_id: params[:user_id], title: params[:title], skill: params[:skill])
    if @moves.size == 1
      flash[:alert] = "これ以上は削除できません"
    else
      unless @move.destroy
  ...
  def limit_destroy
    @moves = Move.where(user_id: params[:user_id], title: params[:title])
    @move = Move.find_by(user_id: params[:user_id], title: params[:title], limit: params[:limit])
    if @moves.size == 1
      flash[:alert] = "これ以上は削除できません"
    else
      unless @move.destroy
